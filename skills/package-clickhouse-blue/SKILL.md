---
name: package-clickhouse-blue
description: Provision and operate a private replicated ClickHouse cluster, Metabase, WireGuard, Cloudflare DNS, and local dbt through colors-compute with Blue.
license: MIT
---
...
uv runs the launcher; `create` and `delete` also need OpenTofu and Ansible.
...
- The installed launcher is a copy, not a symlink. After `npx skills update -p`,
  copy `.agents/skills/package-clickhouse-blue/blue` over the root `./blue`.
