...
METABASE_HOST = "<{ metabase-host }>"
METABASE_PORT = <{ metabase-port }>
PUBLIC_PORTS = (3000, <{ clickhouse-http-port }>, <{ clickhouse-native-port }>, 9181, 9234)
DNS = {
    CLICKHOUSE_HOST: "10.21.0.1",
    "node-1.<{ clickhouse-host }>": "10.21.0.1",
    "node-2.<{ clickhouse-host }>": "10.21.0.2",
    "node-3.<{ clickhouse-host }>": "10.21.0.3",
...
def check_clickhouse():
    ...
    keepers = client.query(
        ...
        "FROM clusterAllReplicas('<{ clickhouse-cluster-name }>', system.zookeeper) "
        "WHERE path='/'"
    ).result_rows[0][0]
