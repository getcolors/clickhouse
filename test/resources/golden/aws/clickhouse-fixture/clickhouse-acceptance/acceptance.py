...
CLICKHOUSE_HOST = "clickhouse.aws.fixture.example"
METABASE_HOST = "metabase.aws.fixture.example"
METABASE_PORT = 3000
PUBLIC_PORTS = (3000, 8123, 9000, 9181, 9234)
DNS = {
    CLICKHOUSE_HOST: "10.21.0.1",
    "node-1.clickhouse.aws.fixture.example": "10.21.0.1",
    "node-2.clickhouse.aws.fixture.example": "10.21.0.2",
    "node-3.clickhouse.aws.fixture.example": "10.21.0.3",
...
def check_clickhouse():
    ...
    for ordinal in range(1, 4):
        peer = clickhouse_connect.get_client(
            ...
            port=8123, username="admin",
            password=os.environ["COLORS_PAR_CLICKHOUSE_ADMIN_PASSWORD"],
        )
        try:
            count = peer.query("SELECT count() FROM clusterAllReplicas('clickhouse-aws', system.one)").result_rows[0][0]
    ...
    request = urllib.request.Request(
        f"http://{CLICKHOUSE_HOST}:8123/?query=SELECT%201",
    ...
    replicas = client.query(
        ...
        "FROM clusterAllReplicas('clickhouse-aws', system.replicas) WHERE database = 'analytics'"
    ).result_rows[0]
    assert tuple(map(int, replicas)) == (6, 0, 3), f"unhealthy replicas: {replicas}"
    keepers = client.query(
        "SELECT countDistinct(hostName()) "
        "FROM clusterAllReplicas('clickhouse-aws', system.zookeeper) "
        "WHERE path='/'"
    ).result_rows[0][0]
