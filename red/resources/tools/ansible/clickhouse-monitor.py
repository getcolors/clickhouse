...
def ping():
    with urllib.request.urlopen('http://127.0.0.1:<{ clickhouse-http-port }>/ping', timeout=10) as response:
...
check('three_replicas', lambda: query("SELECT count() FROM clusterAllReplicas('<{ clickhouse-cluster-name }>', system.one)") == '3')
